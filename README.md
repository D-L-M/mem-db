...
You can also request a list of significant terms from a field in the filtered results by appending the following query string parameters to a search URL: `http://localhost:9999/_search?&significant_terms_field={field_name}&significant_terms_threshold=300&significant_terms_minimum=25`.

In the above example, `{field_name}` is the dot-notation name of the field to get significant terms from, `significant_terms_threshold` is the percentage by which the terms should be significant (in this example, 300% or 3x more common than the background data) and `significant_terms_minimum` is the minimum amount of times a term must occur to be included.
